...
my $total = 0;
my $loops = 0;
for (my ($retstart,$retmax,$jump) = (0,0,100000); $retstart < $count; $retmax += $jump, $retstart = $retmax - $jump) {
    ...
    my $esearch2 = "$http/esearch.fcgi?email=givans\@missouri.edu&tool=gi_fetch&db=$db&retstart=$retstart&retmax=$jump&term=";
    say $esearch2;
